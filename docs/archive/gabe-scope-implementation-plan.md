...
- `[2026-04-21] Phase 7: Integration edits + dogfood doc — DONE. install.sh: added 4 new commands to COMMANDS_ONLY (gabe-scope, -change, -addition, -pivot); template copy extended to include *.yaml + *.json; Option A committed — prompts/ ships to ~/.claude/prompts/gabe-scope/, schemas/ ships to ~/.claude/schemas/gabe-scope/. Scope integration sections appended to 6 existing commands: /gabe-plan reads ROADMAP.md phase + SCOPE.md REQ text + Constraints + Posture with dependency-gate + pivoted-status refusal; /gabe-teach gained `scope` mode teaching SCOPE.md premise + per-phase Why + deep-link by anchor (scope sc-01, scope req-02, scope phase-3); /gabe-align scope-drift checks (uncovered_implementation, phase_goal_unmet, non_goal_violation, constraint_drift) feeding CONCERN bucket; /gabe-review adds req_coverage_gap (MEDIUM), req_inflation (HIGH), scope_bypass (CRITICAL) findings; /gabe-commit audit warns on direct SCOPE.md/ROADMAP.md edits with proper command hint + CHANGES.jsonl exception path + Scope-Bypass-Audit footer; /gabe-push DEPLOYMENTS.md rows get Phase + REQs columns + phase-complete prompt routing through /gabe-scope-change. docs/gabe-scope-v1-dogfood.md: 15-check regression (C01-C15) covering bare-invocation routing, auto-suggest, manual refs × 4 weights × 3 load_modes, brainstorm trigger + 2-cycle cap, research width, authoritative-ref conflict, Step 7.1 coverage block, Step 7.4 re-check on edit, Step 8 archival + tombstone + CHANGES.jsonl + KNOWLEDGE.md, /gabe-scope-change routing for pivot + addition, session pause/resume with markers, --force escape, start-over typed-confirm. Ship bar ≥13/15. Hours: ~2. Blockers: none. All 7 phases complete; /gabe-scope backbone ready for real-project dogfood run.`
